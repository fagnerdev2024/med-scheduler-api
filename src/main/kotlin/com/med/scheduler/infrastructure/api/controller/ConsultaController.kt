...
class ConsultaController(
    ...
    fun agendar(@RequestBody @Valid dados: DadosAgendamentoConsulta): ResponseEntity<DadosDetalhamentoConsulta> {
        return ResponseEntity.ok(consultaUseCase.agendar(dados))
    }

    @DeleteMapping
    @Operation(summary = "Cancela uma consulta existente")
    fun cancelar(@RequestBody @Valid dados: DadosCancelamentoConsulta): ResponseEntity<Void> {
